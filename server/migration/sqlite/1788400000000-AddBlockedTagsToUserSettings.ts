...
/**
 * Per-user content filtering: which TMDB keyword ids a user must not be shown.
 *
 * SQLite cannot add a column to a table with constraints in place without
 * rebuilding it, which is why this follows the same shape as
 * AddDiscordIdsColumn rather than a plain ALTER.
 *
 * The column is nullable with no default, so every existing row reads as "no
 * filter" and nobody's view of the library changes until a tag is assigned.
 */
export class AddBlockedTagsToUserSettings1788400000000
  implements MigrationInterface
{
  name = 'AddBlockedTagsToUserSettings1788400000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO "temporary_user_settings"("id", "locale", "discoverRegion", "streamingRegion", "originalLanguage", "pgpKey", "blockedTags", "discordIds", "pushbulletAccessToken", "pushoverApplicationToken", "pushoverUserKey", "pushoverSound", "telegramChatId", "telegramSendSilently", "watchlistSyncMovies", "watchlistSyncTv", "notificationTypes", "userId", "telegramMessageThreadId") SELECT "id", "locale", "discoverRegion", "streamingRegion", "originalLanguage", "pgpKey", NULL, "discordIds", "pushbulletAccessToken", "pushoverApplicationToken", "pushoverUserKey", "pushoverSound", "telegramChatId", "telegramSendSilently", "watchlistSyncMovies", "watchlistSyncTv", "notificationTypes", "userId", "telegramMessageThreadId" FROM "user_settings"`
    );
